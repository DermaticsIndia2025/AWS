...
export const generateRoutine = async (
    ...
    const prompt = `
        ...
        **Instructions:**
        1.  **Analyze and Select:** Carefully review the user's analysis and goals. From the "Dermatics India Product Catalog", select the MOST appropriate products to build a cohesive AM and PM routine. Pay close attention to the 'suitableFor' tags in the product data to match products to the user's skin conditions.
        2.  **Create the Routine:** Construct a step-by-step AM (morning) and PM (evening) routine. Recommend only the products that are essential for an effective routine based on the user's data. For each step, you must provide:
            ...
            - \`alternatives\`: An array of any suitable alternative products from the catalog that would also work well for this step. For each alternative, provide its \`productId\`, \`variantId\`, \`productName\`, \`productUrl\`, and \`productImageUrl\`. Only include alternatives if they are genuinely good options; it's okay for this array to be empty.
    ...
    const alternativeProductSchema = {
        ...
        required: ["productId", "variantId", "productName", "productUrl", "productImageUrl"]
    };
    ...
    const routineStepSchema = {
        ...
        required: ["productId", "variantId", "productName", "productUrl", "productImageUrl", "purpose", "alternatives"]
    };
