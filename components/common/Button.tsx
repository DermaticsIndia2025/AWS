...
const Button: React.FC<ButtonProps> = ({ as: Component = 'button', variant = 'primary', size = 'md', isLoading = false, children, className, ...props }) => {
  ...
  const variantClasses = {
    ...
    secondary: 'bg-indigo-100 text-brand-primary hover:bg-indigo-200 focus-visible:ring-brand-primary-light focus-visible:ring-offset-brand-bg',
    ghost: 'bg-transparent text-brand-text-muted hover:bg-slate-200/50 hover:text-brand-text-main shadow-none'
  };
